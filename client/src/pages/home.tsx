...
export default function Home() {
  ...
  return (
    <div className={cn(
      ...
      isNotStarted ? "selection:bg-primary/30 [--status-color:var(--primary)]" : isOvertime ? "selection:bg-green-500/30 [--status-color:142_70%_50%]" : "selection:bg-primary/30 [--status-color:var(--primary)]"
    )}>
      {/* Dynamic Theme Background */}
      <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
        <div className={cn(
          "absolute top-0 left-1/4 w-96 h-96 rounded-full blur-[100px] transition-all duration-1000",
          isNotStarted ? "bg-primary/10" : isOvertime ? "bg-green-500/10" : "bg-primary/10"
        )} />
        <div className={cn(
          "absolute bottom-0 right-1/4 w-[500px] h-[500px] rounded-full blur-[100px] transition-all duration-1000",
          isNotStarted ? "bg-blue-500/10" : isOvertime ? "bg-emerald-500/10" : "bg-accent/10"
        )} />
      </div>
      ...
      <main className="relative z-10 flex-1 flex flex-col max-w-full 2xl:max-w-[1600px] mx-auto w-full p-4 gap-4 pb-10 h-full">
        ...
        <header className="flex justify-between items-center h-12 shrink-0">
          <div className="flex items-center gap-3">
            <div className={cn(
              ...
              isNotStarted ? "bg-primary" : isOvertime ? "bg-green-500" : "bg-primary"
            )}>
              <Hourglass className="w-5 h-5 text-white" />
            </div>
            <div>
              <h1 className="text-xl font-bold tracking-tight">ClockOut</h1>
              <p className="text-[10px] text-muted-foreground uppercase tracking-widest leading-none">Because your time matters</p>
            </div>
          </div>
        ...
        {/* Motivational Quote */}
        <motion.div className="min-h-12 shrink-0 bg-card/20 backdrop-blur-md border rounded-xl flex flex-col items-center justify-center px-4 gap-1">
          <div className="flex items-center gap-2">
            <Quote className="w-3 h-3 text-primary shrink-0" />
            <p className="italic text-xs text-muted-foreground font-medium text-center">"{quote.content}"</p>
          </div>
          <p className="text-[10px] text-muted-foreground/60">— {quote.author}</p>
        </motion.div>
        ...
        <div className="flex-1 flex flex-col lg:grid lg:grid-cols-12 gap-4 min-h-0">
          ...
          <div className="lg:col-span-4 xl:col-span-3 flex flex-col min-h-0">
            <div className="flex-1 bg-card/30 backdrop-blur-xl border rounded-2xl p-4 flex flex-col gap-4 overflow-y-auto scrollbar-none">
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  <Label className="text-xs font-bold uppercase tracking-tighter">Calculation Mode</Label>
                  <div className="grid grid-cols-2 gap-2">
                    <Button 
                      variant={calculationMode === "login" ? "default" : "outline"} 
                      size="sm"
                    ...
                    <Button 
                      variant={calculationMode === "planned" ? "default" : "outline"} 
                      size="sm"
                ...
                <AnimatePresence mode="wait">
                  {calculationMode === "login" ? (
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: -10 }}
                    >
                      <TimeInput label="Login Time" value={loginTime} onChange={setLoginTime} />
                    </motion.div>
                  ) : (
                    <motion.div
                      key="planned-input"
                      ...
                      exit={{ opacity: 0, y: -10 }}
                    >
                      <TimeInput label="Planned Out" value={plannedOutTime} onChange={setPlannedOutTime} />
                    </motion.div>
                  )}
                </AnimatePresence>
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <Select value={requiredHours} onValueChange={setRequiredHours}>
                      <SelectTrigger className="h-10 bg-background/50 border-primary/20"><SelectValue /></SelectTrigger>
                      <SelectContent className="max-h-40">{hoursList.map(h => <SelectItem key={h} value={h}>{h}h</SelectItem>)}</SelectContent>
                    </Select>
                    <Select value={requiredMinutes} onValueChange={setRequiredMinutes}>
                      <SelectTrigger className="h-10 bg-background/50 border-primary/20"><SelectValue /></SelectTrigger>
                      <SelectContent className="max-h-40">{minutesList.map(m => <SelectItem key={m} value={m}>{m}m</SelectItem>)}</SelectContent>
                    </Select>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between p-3 bg-background/40 rounded-xl border border-border/50">
                  <div className="space-y-0.5">
                    <Label className="text-xs font-bold uppercase tracking-tighter">Half Day</Label>
                    <p className="text-[10px] text-muted-foreground">Reduce hours by 50%</p>
                  </div>
                  <Checkbox checked={currentSettings.isHalfDay} onCheckedChange={(v) => setSettings({ isHalfDay: !!v })} />
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-2">
                    <Label className="text-xs font-bold uppercase tracking-tighter">Short Leaves</Label>
                    <Select value={currentSettings.shortLeaveCount} onValueChange={(v) => setSettings({ shortLeaveCount: v })}>
                      <SelectTrigger className="h-10 bg-background/50 border-primary/20"><SelectValue /></SelectTrigger>
                      <SelectContent>{shortLeaveCounts.map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
                    </Select>
                  </div>
                  <div className="space-y-2">
                    <Label className="text-xs font-bold uppercase tracking-tighter">Mins/Leave</Label>
                    <Select value={currentSettings.shortLeaveDuration} onValueChange={(v) => setSettings({ shortLeaveDuration: v })}>
                      <SelectTrigger className="h-10 bg-background/50 border-primary/20"><SelectValue /></SelectTrigger>
                      <SelectContent>
                        {["30", "40", "50", "60"].map(m => <SelectItem key={m} value={m}>{m}m</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </div>
                </div>
              ...
              <div className="mt-auto pt-4 border-t border-border/50">
                <Button variant="ghost" size="sm" className="w-full text-[10px] h-8" onClick={handleReset}>
          ...
          <div className="lg:col-span-8 xl:col-span-9 flex flex-col gap-4 overflow-hidden">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 shrink-0">
              {calculationMode === "planned" && (
                <StatCard title="Arrival" value={requiredArrivalTime} subtext="Required" variant="default" className="p-4" />
              )}
              {calculationMode === "login" && (
                <StatCard title="Logout" value={logoutTimeStr} subtext="Calculated" variant="default" className="p-4" />
              )}
              <StatCard title={isOvertime ? "Overtime" : isNotStarted ? "Starts In" : "Remaining"} value={remainingTimeStr} variant={isOvertime ? "success" : "primary"} className="p-4" />
            </div>

            <div className={cn(
              "flex-1 border rounded-2xl p-6 flex flex-col justify-center relative overflow-hidden transition-colors duration-700 min-h-[300px]",
              isNotStarted ? "bg-primary/5 border-primary/20" : isOvertime ? "bg-green-500/10 border-green-500/20" : "bg-card/40 border-border/50"
            )}>
              <div className="relative z-10 text-center lg:text-left">
                <div className="flex items-center justify-center lg:justify-start gap-2 mb-2">
                  <div className={cn("w-2 h-2 rounded-full animate-ping", isNotStarted ? "bg-primary" : isOvertime ? "bg-green-500" : "bg-primary")} />
                  <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground">Live Tracker</span>
                </div>
                
                <h3 className={cn("text-3xl sm:text-4xl lg:text-5xl font-bold mb-4", isNotStarted ? "text-primary" : isOvertime ? "text-green-500" : "text-foreground")}>
                  {isNotStarted ? "Preparing for shift" : isOvertime ? "Great overtime work!" : "Currently in session"}
                </h3>
                
                <p className="text-sm sm:text-base text-muted-foreground max-w-2xl mx-auto lg:mx-0">
                  {isNotStarted ? `Take a breath! You have ${remainingTimeStr} before work begins.` : isOvertime ? `You've exceeded required hours by ${remainingTimeStr}. Take a well-deserved break!` : `Stay focused! You're ${Math.round(progress)}% through your required shift today.`}
                </p>
              </div>
              ...
              <div className="mt-12 max-w-4xl w-full mx-auto lg:mx-0">
                ...
                <div className="h-4 bg-background/50 rounded-full overflow-hidden border border-white/5 relative">
                  <motion.div 
                    className={cn("h-full transition-all duration-1000", isOvertime ? "bg-green-500 shadow-[0_0_20px_rgba(34,197,94,0.3)]" : "bg-primary shadow-[0_0_20px_rgba(var(--primary),0.3)]")}
                    initial={{ width: 0 }}
            ...
            <footer 
              className="min-h-24 shrink-0 bg-card/30 backdrop-blur-xl border rounded-2xl p-4 flex flex-col gap-3 cursor-pointer hover:bg-card/40 transition-colors"
            ...
            >
               <div className="flex items-center justify-between px-1">
                 ...
                 <div className="flex items-center gap-3">
                   ...
                   <span className="text-[8px] text-muted-foreground uppercase">Week starts Monday</span>
                 </div>
               </div>
               <div className="grid grid-cols-7 gap-2 flex-1">
                 {(() => {
                   const startOfCurrentWeek = startOfWeek(toZonedTime(now, IST_TIMEZONE), { weekStartsOn: 1 });
                     ...
                     return (
                       ...
                       >
                         ...
                         <div className={cn(
                           ...
                           isDayHoliday ? "bg-sky-400 text-white border-sky-300 shadow-[0_0_15px_rgba(56,189,248,0.4)]" :
                           isDayToday ? "bg-red-500 text-white border-red-400 shadow-[0_0_15px_rgba(239,68,68,0.4)] scale-110" :
                           isPast ? "bg-green-500 text-white border-green-400" :
                           isUpcoming ? "bg-blue-500/10 text-blue-500 border-blue-500/20" : "bg-muted/20 text-muted border-transparent"
                         )}>
                           {format(day, 'dd')}
                         </div>
                       </div>
                ...
                <div className="p-4">
                  <div className="grid grid-cols-7 gap-1 mb-2">
                    {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(d => (
                      <div key={d} className="text-[10px] font-bold text-muted-foreground text-center uppercase">
                        {d}
                      </div>
                    ))}
                  </div>
                  <div className="grid grid-cols-7 gap-1">
                    {(() => {
                      ...
                      const days = eachDayOfInterval({ start, end: addDays(end, (7 - end.getDay()) % 7) });
                      const today = toZonedTime(now, IST_TIMEZONE);

                      return days.map((day, i) => {
                        const isCurrentMonth = getMonth(day) === getMonth(calendarDate);
                        const isDayToday = isSameDay(day, today);
                        const isPast = day < today && !isDayToday;
                        const isUpcoming = day > today;
                        const isDayHoliday = isHoliday(day);
                        ...
                        return (
                          <div
                            ...
                            className={cn(
                              ...
                              isDayHoliday ? "bg-sky-400 text-white border-sky-300 shadow-lg scale-105 z-10" :
                              isDayToday ? "bg-red-500 text-white border-red-400 shadow-lg scale-110 z-10" :
                              isPast ? "bg-green-500/10 text-green-500 border-green-500/20" :
                              isUpcoming ? "bg-blue-500/5 text-blue-500/50 border-blue-500/10" :
